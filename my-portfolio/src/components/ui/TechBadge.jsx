...
const TechBadge = ({ tech, size = 'md', color = 'gray' }) => {
  ...
  const getColorByTech = (tech) => {
    ...
    if (['javascript', 'typescript', 'python', 'java', 'kotlin', 'c++', 'cplusplus', 'c#', 'csharp', 'php', 'ruby', 'go', 'golang', 'rust', 'swift', 'objective-c', 'json', 'xml', 'assembly'].some(x => t.includes(x))) return 'purple';
    ...
    if (['mysql', 'postgresql', 'postgres', 'sqlite', 'mongodb', 'redis', 'firebase', 'firebase realtime db', 'firestore', 'firebase auth', 'sql', 'nosql', 'local json storage', 'database design', 'tinydb'].some(x => t.includes(x))) return 'green';

    // DEVOPS & INFRASTRUCTURE - Orange
    if (['docker', 'kubernetes', 'jenkins', 'github actions', 'ci/cd', 'aws', 'azure', 'gcp', 'heroku', 'vercel', 'netlify', 'nginx', 'apache', 'tomcat', 'linux', 'ubuntu', 'debian', 'centos', 'bash', 'shell', 'firebase console', 'ssh', 'mcp'].some(x => t.includes(x))) return 'orange';
    ...
    if (['django', 'flask', 'fastapi', 'spring', 'laravel', 'symfony', 'rails', '.net', 'dotnet', 'express', 'nextjs', 'nuxt', 'gatsby', 'sfml', 'scikit-learn', 'pandas', 'numpy', 'ensemble learning', 'tkinter gui integration', 'tkinter', 'langgraph'].some(x => t.includes(x))) return 'cyan';
    ...
    if (['software development', 'information technology', 'it', 'artificial intelligence', 'ai', 'machine learning', 'deep learning', 'cybersecurity', 'security', 'networking', 'cloud computing', 'data science', 'data analytics', 'big data', 'object-oriented programming', 'oop', 'algorithms', 'data structures', 'end-to-end encryption', 'operating systems', 'system calls', 'spinlocks', 'sleeplocks'].some(x => t.includes(x))) return 'indigo';
  ...
  const getIconByTech = (tech) => {
    ...
    if (['mysql', 'postgresql', 'postgres', 'sqlite', 'mongodb', 'redis', 'firebase', 'firebase realtime db', 'firestore', 'sql', 'nosql', 'database design', 'data structures', 'data science', 'data analytics', 'big data', 'tinydb'].some(x => t.includes(x))) return <Database size={14} />;
    if (['git', 'github', 'gitlab', 'bitbucket'].some(x => t.includes(x))) return <GitBranch size={14} />;
    if (['linux', 'ubuntu', 'bash', 'shell', 'terminal', 'docker', 'kubernetes', 'ssh', 'mcp'].some(x => t.includes(x))) return <Terminal size={14} />;
    ...
    if (['python', 'java', 'c++', 'cplusplus', 'c#', 'csharp', 'ruby', 'go', 'golang', 'rust', 'json', 'xml', 'assembly'].some(x => t.includes(x))) return <Code2 size={14} />;
    if (t === 'c') return <Code2 size={14} />;
    if (['django', 'flask', 'spring', 'laravel', 'rails', '.net', 'dotnet', 'scikit-learn', 'pandas', 'numpy', 'ensemble learning', 'fastapi', 'tkinter gui integration', 'tkinter', 'langgraph'].some(x => t.includes(x))) return <Layers size={14} />;
    ...
    if (['information technology', 'it', 'networking', 'operating systems', 'system calls'].some(x => t.includes(x))) return <Monitor size={14} />;
    if (['cybersecurity', 'security', 'end-to-end encryption', 'spinlocks', 'sleeplocks'].some(x => t.includes(x))) return <Shield size={14} />;
