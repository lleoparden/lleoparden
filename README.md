# Hi, I'm Mostafa Eid 👋

Computer Science student at Ain Shams University (Faculty of Computer and Information Sciences, expected 2027) and software developer building full-stack apps, backend systems, and local AI/RAG tooling.

## What I'm doing right now

- **Co-Owner @ [TrustFlow](https://github.com/abyssrift/TrustFlow)** — building the product end to end alongside the project's other founder
- **Software Development Intern @ Orange Business** — Network Software Factory team (Jul–Oct 2025 & Jul–Aug 2026)
- **Freelance web developer** — React, Python, FastAPI
- **Contributing to [Odysseus](https://github.com/arcahyadi/odysseus)** — PewDiePie's open-source AI workspace (merged PRs, applied for a maintainer role)
- **Planning to contribute more open source over the next year** and take part in a few hackathons

## Selected projects

| Project | Stack | Status |
|---|---|---|
| [Local RAG System](https://github.com/lleoparden/RAG-NLP-project) | FastAPI, ChromaDB, Ollama, Docker | Done |
| [Olympus](https://github.com/lleoparden/olympus) | Python, JavaScript, Ollama, Docker | Ongoing |
| SentinelCheck — anti-scam app | React Native, Supabase | Planned (grad capstone) |
| [Job Application Tracking System](https://github.com/lleoparden/job-tracker) | React Native, FastAPI, Firebase | In progress |
| [Kotlin Chat Application](https://github.com/lleoparden/Chat_Application) | Kotlin, Android, Firebase | Done |
| [OOP Chess](https://github.com/lleoparden/oop-chess) | Java, JavaFX | Done |
| SONiC-GUI — AI-assisted network management | React, FastAPI, Redis, WebSockets, Gemini API, LangChain | Done |
| [FOS Educational OS — kernel dev](https://github.com/lleoparden/FOS_TEMPLATE) | C | Done |

Full details: [lleoparden.github.io/lleoparden](https://lleoparden.github.io/lleoparden/)

## Achievements

- Ranked 68th in the ECPC (Egyptian Collegiate Programming Contest) qualification round

## Tech stack

![React](https://img.shields.io/badge/-React-61DAFB?logo=react&logoColor=white&style=flat-square)
![TypeScript](https://img.shields.io/badge/-TypeScript-3178C6?logo=typescript&logoColor=white&style=flat-square)
![Python](https://img.shields.io/badge/-Python-3776AB?logo=python&logoColor=white&style=flat-square)
![FastAPI](https://img.shields.io/badge/-FastAPI-009688?logo=fastapi&logoColor=white&style=flat-square)
![Kotlin](https://img.shields.io/badge/-Kotlin-7F52FF?logo=kotlin&logoColor=white&style=flat-square)
![Java](https://img.shields.io/badge/-Java-007396?logo=openjdk&logoColor=white&style=flat-square)
![Docker](https://img.shields.io/badge/-Docker-2496ED?logo=docker&logoColor=white&style=flat-square)
![Firebase](https://img.shields.io/badge/-Firebase-FFCA28?logo=firebase&logoColor=white&style=flat-square)
![Ollama](https://img.shields.io/badge/-Ollama-000000?logo=ollama&logoColor=white&style=flat-square)
![LangChain](https://img.shields.io/badge/-LangChain-1C3C3C?style=flat-square)

## Contact

- 📧 most.eid.31@gmail.com
- 🌐 [Portfolio](https://lleoparden.github.io/lleoparden/)
- 💻 [GitHub](https://github.com/lleoparden)
- 📍 New Cairo, Cairo, Egypt
